...
#[tauri::command]
pub fn list_projects_by_client(state: State<DbState>, client_id: String) -> AppResult<Vec<Project>> {
    let conn = state.0.lock().map_err(|e| crate::error::AppError::Database(
        rusqlite::Error::InvalidParameterName(e.to_string()),
    ))?;
...
#[tauri::command]
pub fn update_project(state: State<DbState>, id: String, input: UpdateProject) -> AppResult<Project> {
    let conn = state.0.lock().map_err(|e| crate::error::AppError::Database(
        rusqlite::Error::InvalidParameterName(e.to_string()),
    ))?;
